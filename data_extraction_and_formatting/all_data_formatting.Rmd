...
##this is currently only selecting Groceries and Meals 
comm_resource_data <- 
  read_sheet(comm_resource_key, skip = 1) %>% 
  drop_na(type) %>% 
  mutate(
    resource = type %>% str_to_lower() %>% str_replace(" ", "_")
  )
...
legal_resource_data <-
  ...
  select(-1, county = ...2) %>% 
  mutate(
    notes = if_else( is.na(notes), notes, paste(notes, other_notes, sep = ". ")),
    notes = str_remove(notes, "NA") ##Make this more eloquent--> works for now 
  )
