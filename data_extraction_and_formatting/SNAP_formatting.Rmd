...
wic_only <- 
  retailers %>% 
  filter(Type == "WIC_only_store")
```

```{r}
str_c(c("Only serves WIC-customers.", notes), " ")
wic_only %>% 
  transmute(
    notes = if_else(Type == "WIC_only_store", str_c(str_replace_na(c(notes, "Only serves")), collapse = " "), notes),
  )

str_c(c("hello", "there"), collapse = " ")
```
...
wic_only %>% 
  transmute(
    ...
    offers = '',
    notes = if_else(Type == "WIC_only_store", str_c("Only serves WIC-customers. ", notes), notes),
    web_link = web_link,
    twitter = '',
    instagram = '',
    facebook = facebook,
