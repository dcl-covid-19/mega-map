...
## Transforming the Data  (SNAP and LEGAL)
```{r}
iterate_sets <- function(data_set, data_set_name) {
  ...
  if (data_set_name == "SNAP_data") {
    output_sheet <-
      output_sheet %>% 
      mutate(
        status = data_set$status %>% replace_na(1) %>% as.numeric(),
...
temp <-  
  ...
  bind_cols(
    bind_cols(
      data_set$days_hours %>%
        map_dfr(process_hours_cr, senior = T)
    )
  ) %>% 
  mutate(
    region = 
...
output_sheet <-
  ...
  mutate(
    ...
    free_produce = if_else(data_set$type == "grocery", 1, 0),
    free_groceries = if_else(data_set$type  == "grocery", 1, 0), 
    public = if_else(str_detect(data_set$eligibility, "Public") == TRUE, 1, 0),
