...
school_meal_data <-
  read_sheet(
    ...
    sheet = "School Sites",
    skip = 1,
    col_types = "cccdddccccddccccccccDcDccccdc"
  ) %>% 
  mutate(
...
SNAP_output <- output_sheet %>% select(all_of(output_variable_names)) %>% 
  mutate_at(vars(mon:sp_sun), ~if_else(str_detect(., "CLOSED|Closed|closed|No"), "0" , .))
```
...
SNAP_output %>% 
  write_sheet(ss = "1SOUonPM40Si4LrPoGrkUnujbmLBxCQe1SdxydH2dVI4", sheet = "SNAP")

farmers_output %>% 
  write_sheet(ss = "1SOUonPM40Si4LrPoGrkUnujbmLBxCQe1SdxydH2dVI4", sheet = "Farmers Market" )
